...
def _current_agent_runtime() -> str:
    provider = os.getenv("LLM_PROVIDER", "sage").strip().lower()
    return "maple" if provider == "maple" else "sage"


def _ensure_session_runtime_snapshot(session: dict) -> None:
    if not session.get("agent_runtime"):
        session["agent_runtime"] = _current_agent_runtime()
...
def _get_or_create_session(session_id: str, user: dict) -> dict:
    ...
    with _sessions_lock:
        ...
        _sessions[session_id] = {
            ...
            "owner_id": owner_id,
            "agent_runtime": _current_agent_runtime(),
...
async def delete_session(session_id: str, user: dict = Depends(auth.require_admin_or_approved_user)):
    ...
    with _sessions_lock:
        if session_id in _sessions:
            ...
            if not _can_access_session(user, session):
                raise HTTPException(status_code=403, detail="Session access denied")
            _ensure_session_runtime_snapshot(session)
