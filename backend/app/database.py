...
def init_schema():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS lifecycle_deployment_surface_acknowledgements (
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS lifecycle_retention_policies (
            lifecycle_data_class TEXT PRIMARY KEY,
...
def list_lifecycle_deployment_surface_acknowledgements() -> dict[str, dict]:
    with get_cursor() as cursor:
        cursor.execute("SELECT * FROM lifecycle_deployment_surface_acknowledgements")
        rows = cursor.fetchall()
    return {row["surface_key"]: dict(row) for row in rows}


def acknowledge_lifecycle_deployment_surface(surface_key: str, acknowledged_by: str) -> dict:
    now = datetime.now(timezone.utc).isoformat()
    with get_write_cursor() as cursor:
        cursor.execute(
            """
            INSERT INTO lifecycle_deployment_surface_acknowledgements (
                surface_key,
                acknowledged_by,
                acknowledged_at
            )
            ...
            """,
            (surface_key, acknowledged_by, now),
        )
    return {
...
def _retention_policy_from_row(row) -> dict:
    return {
        "lifecycle_data_class": row["lifecycle_data_class"],
...
def default_lifecycle_retention_policy(lifecycle_data_class: str) -> dict:
    return {
...
def get_lifecycle_retention_policies(lifecycle_data_classes: list[str]) -> dict[str, dict]:
    if not lifecycle_data_classes:
        return {}
    placeholders = ",".join("?" for _ in lifecycle_data_classes)
    with get_cursor() as cursor:
        cursor.execute(
            f"""
            SELECT * FROM lifecycle_retention_policies
            WHERE lifecycle_data_class IN ({placeholders})
            """,
            lifecycle_data_classes,
        )
        rows = cursor.fetchall()
    policies = {
        lifecycle_data_class: default_lifecycle_retention_policy(lifecycle_data_class)
        for lifecycle_data_class in lifecycle_data_classes
    }
    for row in rows:
        policies[row["lifecycle_data_class"]] = _retention_policy_from_row(row)
    return policies


def upsert_lifecycle_retention_policy(
    lifecycle_data_class: str,
    *,
    enabled: bool,
    retention_days: int,
    ...
    with get_write_cursor() as cursor:
        cursor.execute(
            """
            INSERT INTO lifecycle_retention_policies (
                lifecycle_data_class,
                enabled,
                retention_days,
                schedule_enabled,
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            ON CONFLICT(lifecycle_data_class) DO UPDATE SET
...
def list_active_user_memories_created_before(
    cutoff: datetime,
    limit: int = 1000,
    offset: int = 0,
) -> list[dict]:
    """List active User Memory records eligible for age-based retention."""
    bounded_limit = max(0, min(int(limit), 5000))
    bounded_offset = max(0, int(offset))
    cutoff_value = cutoff.strftime("%Y-%m-%d %H:%M:%S") if isinstance(cutoff, datetime) else str(cutoff)
    with get_cursor() as cursor:
        cursor.execute("""
            SELECT
                id,
            ...
            LIMIT ? OFFSET ?
        """, (cutoff_value, bounded_limit, bounded_offset))
        return [_user_memory_row_to_dict(row) for row in cursor.fetchall()]
