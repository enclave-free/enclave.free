  ...
  render(
    <ThemeProvider>
      <InstanceConfigProvider>
        <ConversationSurface turns={turns} onSend={onSend} isRunning={options.isRunning} />
      </InstanceConfigProvider>
    </ThemeProvider>
  )
  return { onSend }
...
describe('ConversationSurface', () => {
  ...
  it('renders activity steps before the assistant answer and sends prompts', async () => {
    ...
    expect(screen.getByText('Database results were redacted from the trace.')).toBeInTheDocument()

    await user.type(screen.getByRole('textbox', { name: 'Ask anything...' }), 'Next question')
  ...
  it('shows the empty Conversation state before any turns exist', () => {
    ...
    expect(
      screen.getByText('Ask questions about your knowledge base or start a general conversation')
    ).toBeInTheDocument()
  })
