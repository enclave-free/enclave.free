...
export function AdminAIConfig() {
  ...
  const [selectedUserTypeId, setSelectedUserTypeId] = useState<number | null>(
    null
  );
  ...
  const handleSave = async () => {
    ...
    const item =
      aiConfig?.prompt_sections.find((i) => i.key === editingKey) ||
  ...
  const handlePreview = async () => {
    ...
    try {
      const sampleQuestion = t(
        'promptPreview.sampleQuestion',
      ...
      setPreviewError(
        err instanceof Error ? err.message : t('errors.failedToPreviewPrompt')
      );
    }
  };
  ...
  const handleDocumentToggle = async (
    ...
    field: 'is_available' | 'is_default_active'
  ) => {
    setToggleError(null);
    try {
      if (selectedUserTypeId !== null) {
  ...
  const PARAMETERS_HELP_PAGES = [
    {
      title: t(
        'adminAI.parametersHelp.overviewTitle',
  ...
  const PROMPT_HELP_PAGES = [
    ...
    {
      title: t(
        'adminAI.promptHelp.placeholdersTitle',
  ...
  const renderConfigItem = (item: AIConfigItem | AIConfigWithInheritance) => {
    ...
    const userDefaultToolIds =
      item.key === 'user_default_tool_ids'
        ? parseUserDefaultToolIds(editValue)
        : [];
        ...
        {isEditing ? (
            ...
            {item.key === 'user_default_tool_ids' ? (
              <div className="flex flex-wrap gap-2">
                {userDefaultToolOptions.map((option) => {
                  ...
                  return (
                    <label
                      key={option.id}
                      className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => {
                          const next = checked
                            ? userDefaultToolIds.filter(
                                (id) => id !== option.id
                              )
                            : [...userDefaultToolIds, option.id];
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-mono text-text w-12 text-right">
                  {(parseFloat(editValue) || 0).toFixed(
                    item.key === 'temperature' ? 1 : 0
                  )}
                </span>
              </div>
            ) : (
          ...
          <div className="mt-2">
            {item.key === 'knowledge_source_default' ? (
              ...
              <span
                className={`text-sm ${item.value === 'true' ? 'text-success' : 'text-text-muted'}`}
              >
                {item.value === 'true'
                  ? t('common.enabled', 'Enabled')
                  : t('common.disabled', 'Disabled')}
  ...
  return (
    <OnboardingCard
      ...
      subtitle={t(
        'adminAI.subtitle',
        'Configure Sage behavior, prompts, and session defaults'
      )}
      footer={footer}
    >
      <div className="space-y-6">
          ...
          <div className="bg-surface-overlay border border-border rounded-xl p-4">
            ...
            {selectedUserTypeId !== null && (
              <p className="text-xs text-text-muted mt-3">
                {t(
                  'adminAI.overrideHint',
                  'Changes made here will override global settings for this user type. Items marked "Override" have custom values; items marked "Inherited" use global defaults.'
                )}
              </p>
            )}
          </div>
        ...
        {aiError && (
          <Callout
            label={t('adminAI.loadErrorLabel', 'Agent Settings load error')}
            tone="error"
          >
        ...
        {previewError && (
          <Callout
            label={t(
              'adminAI.previewErrorLabel',
              'Agent Settings preview error'
            )}
            tone="error"
          >
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-error">
                {translateMaybeKey(previewError)}
              </p>
            </div>
          </Callout>
        )}
        ...
        {toggleError && (
          <Callout
            label={t(
              'adminAI.documentToggleErrorLabel',
              'Agent Settings document toggle error'
            )}
            tone="error"
          >
            <div className="flex items-center gap-2">
        ...
        <div className="card card-sm p-5! bg-surface-overlay!">
          ...
          <p className="text-sm text-text-secondary mb-1">
            {t(
              'adminAI.promptTemplateDesc',
              'Shape how the AI communicates with your users'
            )}
          </p>
          <p className="text-xs text-text-muted mb-4">
            {t(
              'adminAI.promptTemplateHint',
              "These settings define the AI's personality and boundaries. Changes take effect immediately for new conversations."
            )}
          </p>
        ...
        <div className="card card-sm p-5! bg-surface-overlay!">
          <h3 className="heading-sm mb-2 flex items-center gap-2">
            ...
            <button
              ...
              className="ml-1 text-text-muted hover:text-accent transition-colors"
              aria-label={t(
                'adminAI.parametersHelp.ariaLabel',
                'Response settings help'
              )}
            >
              <HelpCircle className="w-5 h-5" />
            </button>
          ...
          <p className="text-xs text-text-muted mb-4">
            {t(
              'adminAI.parametersHint',
              'These technical settings affect response speed and quality. The defaults work well for most use cases — only adjust if you notice issues.'
            )}
          </p>
        ...
        <div className="card card-sm p-5! bg-surface-overlay!">
          ...
          <p className="text-sm text-text-secondary mb-1">
            {t(
              'adminAI.sessionDefaultsDesc',
              'Set which features are on by default'
            )}
          </p>
          <p className="text-xs text-text-muted mb-4">
            {t(
              'adminAI.sessionDefaultsHint',
              'Control which Agent Runtime capabilities are enabled when users start a new Conversation.'
            )}
          </p>
        ...
        <div className="card card-sm p-5! bg-surface-overlay!">
          ...
          <p className="text-sm text-text-secondary mb-1">
            {t(
              'adminAI.documentDefaultsDesc',
              'Control which documents the AI can reference'
            )}
          </p>
          <p className="text-xs text-text-muted mb-4">
            {t(
              'adminAI.documentDefaultsHintV2',
              'Documents marked "Available" can be used by Knowledge Search. Documents marked "Active by Default" are included when Knowledge Sources is set to selected default documents.'
            )}
          </p>
          ...
          {scopedDocuments.length > 0 ? (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          {isOverride && selectedUserTypeId !== null && (
                            <button
                              type="button"
                              onClick={() =>
                                handleRevertDocOverride(doc.job_id)
                              }
                              className="text-xs text-text-muted hover:text-accent transition-colors flex items-center gap-1"
                              title={t(
                                'adminAI.revertToGlobal',
                                'Revert to global'
                              )}
                              aria-label={t(
                                'adminAI.revertToGlobal',
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <button
                              type="button"
                              onClick={() =>
                                handleDocumentToggle(doc, 'is_available')
                              }
                          ...
                          <div className="flex items-center gap-2 text-xs">
                            <button
                              type="button"
                              onClick={() =>
                                handleDocumentToggle(doc, 'is_default_active')
                              }
            ...
            <div className="text-center py-6 bg-surface border border-border border-dashed rounded-lg">
              ...
              />
              <p className="text-xs text-text-muted">
                {t('adminAI.noDocuments', 'No documents uploaded yet')}
              </p>
              <Link
                to="/admin/upload"
        ...
        >
          <div className="bg-surface border border-border rounded-xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 id="preview-modal-title" className="font-semibold text-text">
                {t('adminAI.promptPreview', 'Prompt Preview')}
              </h3>
              <button
      ...
      {showParametersHelpModal && (
        <div
          ...
          onKeyDown={(e) =>
            e.key === 'Escape' && handleCloseParametersHelpModal()
          }
          tabIndex={-1}
        >
          <div className="bg-surface border border-border rounded-xl p-6 w-full max-w-lg mx-4 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3
                id="parameters-help-modal-title"
            ...
            <div className="min-h-[280px]">
              {PARAMETERS_HELP_PAGES[parametersHelpPage].content ===
              'overview' ? (
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.parametersHelp.overviewDesc',
                      'These settings control how the AI generates responses. They affect creativity, length, and randomness of outputs.'
                    )}
                  </p>
                  <div className="bg-surface-overlay border border-border rounded-lg p-4">
                    <p className="text-sm font-medium text-text mb-2">
                      {t(
                        'adminAI.parametersHelp.whenToChange',
                    ...
                    <ul className="text-xs text-text-muted space-y-2 list-disc list-inside">
                      <li>
                        {t(
                          'adminAI.parametersHelp.tip1',
                          'Responses too short? Increase max tokens'
                        )}
                      </li>
                      <li>
                        {t(
                          'adminAI.parametersHelp.tip2',
                          'Responses too random? Lower temperature'
                        )}
                      </li>
                      <li>
                        {t(
                          'adminAI.parametersHelp.tip3',
                          'Responses too repetitive? Raise temperature slightly'
                        )}
                      </li>
                      <li>
                        {t(
                          'adminAI.parametersHelp.tip4',
                          'Need consistent answers? Use temperature 0'
                        )}
                      </li>
                    </ul>
                  </div>
                  <p className="text-xs text-text-muted">
                    {t(
                      'adminAI.parametersHelp.defaultNote',
                      'The defaults work well for most use cases. Only adjust if you notice specific issues.'
                    )}
                  </p>
                </div>
              ) : PARAMETERS_HELP_PAGES[parametersHelpPage].content ===
                ...
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.parametersHelp.temperatureDesc',
                      'Temperature controls creativity vs consistency. Think of it as a "randomness dial".'
                    )}
                  </p>
                  <div className="space-y-2">
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-accent">
                          {t(
                            'adminAI.parametersHelp.deterministic',
                            'Deterministic'
                          )}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted">
                        {t(
                          'adminAI.parametersHelp.temp0',
                          'Always picks the most likely response. Use for factual Q&A, data extraction.'
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-success">
                          {t('adminAI.parametersHelp.balanced', 'Balanced')}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted">
                        {t(
                          'adminAI.parametersHelp.tempMid',
                          'Good balance of consistency and natural variation. Recommended for most uses.'
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-warning">
                          {t('adminAI.parametersHelp.creative', 'Creative')}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted">
                        {t(
                          'adminAI.parametersHelp.tempHigh',
                          'More creative, varied responses. Use for brainstorming, creative writing.'
                        )}
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.parametersHelp.tokensDesc',
                      'These settings control response length and sampling behavior.'
                    )}
                  </p>
                  <div className="space-y-2">
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      ...
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.parametersHelp.maxTokensDesc',
                          'Maximum length of the response. 1 token is roughly 4 characters or 3/4 of a word. Higher values allow longer responses but cost more.'
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      ...
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.parametersHelp.topPDesc',
                          'Nucleus sampling: only consider tokens whose cumulative probability is within this threshold. 0.9-1.0 is typical. Lower = more focused.'
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      ...
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.parametersHelp.topKDesc',
                          'Only consider the top K most likely tokens. 0 = disabled (use top_p instead). Lower values = more focused responses.'
                        )}
                      </p>
                    </div>
                  </div>
                  <p className="text-xs text-text-muted mt-3">
                    {t(
                      'adminAI.parametersHelp.samplingNote',
                      'Tip: Adjust either temperature OR top_p, not both at once.'
                    )}
                  </p>
                </div>
              )}
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
              <button
                onClick={() =>
                  setParametersHelpPage((prev) => Math.max(0, prev - 1))
                }
              ...
              <button
                onClick={() =>
                  setParametersHelpPage((prev) =>
                    Math.min(PARAMETERS_HELP_PAGES.length - 1, prev + 1)
                  )
                }
                disabled={
                  parametersHelpPage === PARAMETERS_HELP_PAGES.length - 1
                }
                className="flex items-center gap-1 text-sm text-text-muted hover:text-text disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
                {t('common.next', 'Next')}
        ...
        >
          <div className="bg-surface border border-border rounded-xl p-6 w-full max-w-lg mx-4 shadow-xl">
            ...
            <div className="min-h-[280px]">
              {PROMPT_HELP_PAGES[promptHelpPage].content === 'overview' ? (
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.promptHelp.overviewDesc',
                      "Prompt templates define how the AI responds. They're the instructions that shape its personality and behavior."
                    )}
                  </p>
                  <div className="space-y-2">
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <p className="text-sm font-medium text-text">
                        {t('adminAI.promptHelp.systemPrompt', 'System Prompt')}
                      </p>
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.promptHelp.systemPromptDesc',
                          "The AI's core identity and rules. Defines personality, tone, and boundaries."
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <p className="text-sm font-medium text-text">
                        {t(
                          'adminAI.promptHelp.contextTemplate',
                          'Context Template'
                        )}
                      </p>
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.promptHelp.contextTemplateDesc',
                          'How retrieved documents are formatted and presented to the AI.'
                        )}
                      </p>
                    </div>
                    <div className="bg-surface-overlay border border-border rounded-lg p-3">
                      <p className="text-sm font-medium text-text">
                        {t(
                          'adminAI.promptHelp.queryTemplate',
                          'Query Template'
                        )}
                      </p>
                      <p className="text-xs text-text-muted mt-1">
                        {t(
                          'adminAI.promptHelp.queryTemplateDesc',
                          'How user questions are formatted before sending to the AI.'
                        )}
                      </p>
                    </div>
                  </div>
                </div>
              ) : PROMPT_HELP_PAGES[promptHelpPage].content ===
                'placeholders' ? (
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.promptHelp.placeholdersDesc',
                      'Use these placeholders in your templates. They get replaced with actual values at runtime.'
                    )}
                  </p>
                  <div className="bg-surface-overlay border border-border rounded-lg p-3 font-mono text-xs space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-text-muted">
                        {t(
                          'adminAI.promptHelp.contextPlaceholder',
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-text-muted">
                        {t(
                          'adminAI.promptHelp.queryPlaceholder',
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-text-muted">
                        {t(
                          'adminAI.promptHelp.historyPlaceholder',
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className="text-text-muted">
                        {t(
                          'adminAI.promptHelp.userInfoPlaceholder',
                  ...
                  <p className="text-xs text-text-muted mt-3">
                    {t(
                      'adminAI.promptHelp.placeholdersNote',
                      'Missing placeholders will be replaced with empty strings.'
                    )}
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  <p className="text-sm text-text-muted mb-4">
                    {t(
                      'adminAI.promptHelp.tipsDesc',
                      'Guidelines for modifying prompts safely and effectively:'
                    )}
                  </p>
                  <div className="space-y-2">
                    <div className="bg-success/10 border border-success/20 rounded-lg p-3">
                      <p className="text-sm font-medium text-success mb-1">
                        {t('adminAI.promptHelp.safeChanges', 'Safe to change:')}
                      </p>
                      <ul className="text-xs text-text-muted space-y-1 list-disc list-inside">
                        <li>
                          {t(
                            'adminAI.promptHelp.safe1',
                            'Tone and personality descriptions'
                          )}
                        </li>
                        <li>
                          {t(
                            'adminAI.promptHelp.safe2',
                            'Adding domain-specific instructions'
                          )}
                        </li>
                        <li>
                          {t(
                            'adminAI.promptHelp.safe3',
                            'Response format preferences'
                          )}
                        </li>
                        <li>
                          {t(
                            'adminAI.promptHelp.safe4',
                    ...
                    <div className="bg-warning/10 border border-warning/20 rounded-lg p-3">
                      <p className="text-sm font-medium text-warning mb-1">
                        {t(
                          'adminAI.promptHelp.riskyChanges',
                          'Be careful with:'
                        )}
                      </p>
                      <ul className="text-xs text-text-muted space-y-1 list-disc list-inside">
                        <li>
                          {t(
                            'adminAI.promptHelp.risky1',
                            'Removing placeholders (context, query)'
                          )}
                        </li>
                        <li>
                          {t(
                            'adminAI.promptHelp.risky2',
                            'Changing the response structure significantly'
                          )}
                        </li>
                        <li>
                          {t(
                            'adminAI.promptHelp.risky3',
                            'Very long prompts (may hit token limits)'
                          )}
                        </li>
                      </ul>
                    </div>
                  ...
                  <p className="text-xs text-text-muted mt-3">
                    {t(
                      'adminAI.promptHelp.previewTip',
                      'Use the Preview button to see how your complete prompt looks before saving.'
                    )}
                  </p>
                </div>
              )}
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-border">
              <button
                onClick={() =>
                  setPromptHelpPage((prev) => Math.max(0, prev - 1))
                }
              ...
              <button
                onClick={() =>
                  setPromptHelpPage((prev) =>
                    Math.min(PROMPT_HELP_PAGES.length - 1, prev + 1)
                  )
                }
