...
import { ArrowLeft, Plus, Pencil, Trash2, Loader2, LifeBuoy, ShieldCheck, X } from 'lucide-react'
import {
  Badge,
  Button,
  Callout,
...
const CONTACT_KEYS = ['phone', 'email', 'url', 'secure_channel', 'address', 'notes'] as const
...
function statusTone(status: Resource['status']): 'success' | 'warning' | 'neutral' {
...
export function AdminResourcesDirectory() {
  ...
  const buildBody = () => {
    ...
    const body: Record<string, unknown> = {
      ...
      scope_code: form.scope_level === 'global' ? null : form.scope_code.trim() || null,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const endpoint = editing ? `/admin/resources/${editing.resource_id}` : '/admin/resources'
      const method = editing ? 'PUT' : 'POST'
      const response = await adminFetch(endpoint, { method, body: JSON.stringify(body) })
      if (!response.ok) {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      const response = await adminFetch(`/admin/resources/${deleteTarget.resource_id}`, { method: 'DELETE' })
    ...
    <PageShell
      ...
      header={
        <div className="flex flex-col gap-4">
          <Link to="/admin" className="inline-flex items-center gap-1 text-sm text-text-secondary hover:text-text">
    ...
    >
      ...
      {loading ? (
        ...
        <Card className="py-12 text-center text-text-secondary">
          {t('adminResources.empty', 'No resources yet. Add one, or describe it to the admin assistant in chat.')}
        </Card>
      ) : (
        <div className="flex flex-col gap-3">
          {resources.map((resource) => (
            <Card key={resource.resource_id} className="flex flex-col gap-2 p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-medium text-text">{resource.name || resource.resource_id}</span>
                  ...
                  <div className="mt-1 text-sm text-text-secondary">
                    {[resource.resource_type, resource.coverage].filter(Boolean).join(' · ')}
                  </div>
                  ...
                  {resource.status === 'pending' && resource.missing_fields.length > 0 && (
                    <div className="mt-1 text-xs text-warning">
                      {t('adminResources.missing', 'Missing to go live')}: {resource.missing_fields.join(', ')}
                    </div>
                  )}
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  <IconButton label={t('common.edit', 'Edit')} onClick={() => openEdit(resource)}>
                    <Pencil className="h-4 w-4" />
                  </IconButton>
                  <IconButton label={t('common.delete', 'Delete')} onClick={() => setDeleteTarget(resource)}>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/50 p-4">
          <Card className="my-8 w-full max-w-2xl">
            <div className="flex items-center justify-between border-b border-border px-5 py-3">
              ...
              <IconButton label={t('common.close', 'Close')} onClick={closeForm}>
                <X className="h-4 w-4" />
              </IconButton>
            </div>
            <div className="flex flex-col gap-4 px-5 py-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <TextField
                  label={t('adminResources.languages', 'Languages (comma-separated ISO codes)')}
              ...
              <Textarea
                ...
                rows={2}
                description={t('adminResources.descriptionHint', 'What the assistant should tell the person about this resource.')}
              />
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <TextField
                  ...
                  onChange={(e) => setForm({ ...form, scope_code: e.target.value })}
                  disabled={form.scope_level === 'global'}
                  placeholder={form.scope_level === 'subregion' || form.scope_level === 'region' ? 'M49 e.g. 013' : 'ISO e.g. NI'}
                  description={t(
              ...
              <div>
                <div className="mb-1 text-sm font-medium text-text">{t('adminResources.helpTypes', 'Types of help')}</div>
              ...
              <div>
                <div className="mb-1 text-sm font-medium text-text">{t('adminResources.contact', 'Contact methods')}</div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <TextField
                  ...
                  onChange={(e) => setForm({ ...form, vetted_by: e.target.value })}
                />
                <TextField
              ...
              <div className="flex flex-wrap gap-6">
                <label className="flex items-center gap-2 text-sm text-text">
                  ...
                  />
                  {t('adminResources.markVerified', 'Mark as verified (ranks first)')}
                </label>
                <label className="flex items-center gap-2 text-sm text-text">
                  <input
                    type="checkbox"
