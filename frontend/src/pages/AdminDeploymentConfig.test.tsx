...
describe('AdminDeploymentConfig', () => {
    ...
    mockAdminFetch.mockImplementation(
      (endpoint: string, options?: RequestInit) => {
        ...
        if (endpoint === '/admin/deployment/runtime-env/sage') {
          return Promise.resolve(
            new Response('TINFOIL_MODEL=kimi-k2-6\n', {
        ...
        if (endpoint === '/admin/deployment/runtime-env/core-backend') {
          return Promise.resolve(
            new Response('LLM_MODEL=kimi-k2-6\n', {
              headers: { 'content-type': 'text/plain' },
            })
          );
        }
            ...
            Response.json({
              ...
              runtime_env: {
                sage: {
                  ...
                  generated: {
                    status: 'current',
                    latest_export_at: '2026-05-19T12:00:00+00:00',
                  },
                  running: {
                    status: 'not_directly_introspected',
                    summary:
                      'Sage live runtime env is not directly introspected in this slice; use service health plus generated env freshness.',
                    changed_keys_requiring_restart: [],
                  },
                },
                core_backend: {
            ...
            Response.json({
              ...
              items: [
                ...
                },
                {
                  ...
                  summary:
                    'Record restore drill evidence in the operator checklist.',
                  next_action:
                    'Record a restore drill for the Single-Instance Deployment.',
                  conversation_blocking: false,
                },
                {
        ...
        if (
          endpoint === '/admin/deployment/inference-verification/records/42'
        ) {
          ...
          return Promise.resolve(
            Response.json({
              ...
              content_encryption: {
                status: 'configured',
                summary:
                  'Content Encryption Key is configured for backend-readable active content storage.',
              },
              artifact_encryption: {
              ...
              retention_scheduler: {
                ...
                summary:
                  'Scheduled Retention Policy marks classes for retention; this prototype does not include its own Retention Scheduler.',
                observation: {
                  status:
                    sessionMemoryRetentionPolicy.scheduled_enforcement_enabled
                      ? 'stale'
                      : 'disabled',
                  enabled_classes:
                    sessionMemoryRetentionPolicy.scheduled_enforcement_enabled
              ...
              data_classes: [
                {
                  ...
                  deletion: {
                    status: 'complete',
                    summary:
                      'Conversation deletion removes the public session record and associated Sage Session Memory.',
                  },
                  retention: {
                    status: 'partial',
                    summary:
                      'Operators can invoke retention execution for stale active Conversation state; persistent Sage Session Memory retention is not implemented.',
                  },
                  audit: {
                    status: 'not_started',
                    summary:
                      'Session Memory lifecycle actions are not yet represented in the Audit Log.',
                  },
                  confidentiality: {
                    status: 'partial',
                    summary:
                      'Persistent Session Memory confidentiality is owned by Sage.',
                  },
                  retention_policy: {
                    lifecycle_data_class: 'sage_session_memory',
                    ...sessionMemoryRetentionPolicy,
              ...
              secure_erase: {
                status: 'unsupported',
                summary:
                  'Secure Erase is out of scope for v1; lifecycle controls apply to stated active-storage targets and exclude unsupported Deployment Surfaces.',
              },
              unsupported_deployment_surfaces: [
                {
                  ...
                  summary:
                    'Container stdout/stderr logs are managed by the deployment runtime, not product lifecycle controls.',
                  status: 'unsupported',
                ...
                },
                {
                  ...
                  summary:
                    'SQLite write-ahead-log files are database runtime artifacts.',
                  status: 'unsupported',
                  acknowledged: false,
                },
              ],
              unsupported_deployment_surface_categories: [
                {
                  ...
                  status: 'unsupported',
                  guidance:
                    'Configure deployment log retention outside the product.',
                  acknowledged:
                    acknowledgedSurfaceCategories.includes('runtime_logs'),
                  acknowledged_by: acknowledgedSurfaceCategories.includes(
                    'runtime_logs'
                  )
                    ? 'admin-pubkey'
                    : null,
                  acknowledged_at: acknowledgedSurfaceCategories.includes(
                    'runtime_logs'
                  )
                    ? '2026-05-17T12:00:00Z'
                    : null,
                  posture_version: acknowledgedSurfaceCategories.includes(
                    'runtime_logs'
                  )
                    ? 'posture-version'
                    : null,
                  surfaces: [
                    {
                      ...
                      status: 'unsupported',
                      acknowledged:
                        acknowledgedSurfaceKeys.includes('docker_logs'),
                    },
                  ],
                },
                {
                  ...
                  status: 'unsupported',
                  guidance:
                    'Manage WAL and database maintenance artifacts through database operator policy.',
                  acknowledged: false,
        ...
        if (endpoint === '/admin/lifecycle/artifact-encryption-posture') {
          ...
          artifactEncryptionPosture =
            body.posture === 'disabled' ? 'disabled' : 'required';
          return Promise.resolve(
            Response.json({
              artifact_encryption: {
                posture: artifactEncryptionPosture,
        ...
        if (
          ...
          const body = JSON.parse(String(options?.body ?? '{}'));
          acknowledgedSurfaceKeys =
            body.acknowledged === true ? ['docker_logs'] : [];
          return Promise.resolve(
            Response.json({
              unsupported_deployment_surfaces: [
                {
        ...
        if (
          ...
          const body = JSON.parse(String(options?.body ?? '{}'));
          acknowledgedSurfaceCategories =
            body.acknowledged === true ? ['runtime_logs'] : [];
          return Promise.resolve(
            Response.json({
              unsupported_deployment_surface_categories: [],
            })
        ...
        if (
          endpoint === '/admin/lifecycle/retention-policies/sage_session_memory'
        ) {
        ...
        if (endpoint === '/admin/lifecycle/deletion-tombstones/7/retry') {
          if (tombstoneRetryShouldFail) {
            return Promise.resolve(
              Response.json({ detail: 'completed' }, { status: 409 })
            );
          }
        ...
        if (endpoint.startsWith('/admin/lifecycle/deletion-tombstones')) {
          ...
          const tombstones = tombstonesFixture ?? [
            makeTombstoneFixture(tombstoneRetryCompleted),
          ];
  ...
  it('shows rate limit parameters with operator-friendly labels', async () => {
    ...
    const securitySettings = screen.getByRole('group', {
      name: 'Security & URLs Settings',
    });
    expect(
      within(securitySettings).getByText('Chat Rate Limit')
    ).toBeInTheDocument();
  });
  ...
  it('lets admins export a Sage runtime env artifact from Deployment Settings', async () => {
    ...
    await user.click(
      screen.getAllByRole('button', { name: 'Export Sage env' })[0]
    );

    await waitFor(() => {
      expect(mockAdminFetch).toHaveBeenCalledWith(
        '/admin/deployment/runtime-env/sage'
      );
    });
  });
  ...
  it('shows Sage env apply guidance after export without implying Sage changed live', async () => {
    ...
    expect(
      screen.getByText('Treat sage.env as sensitive deployment material.')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it('lets admins export a core-backend runtime env artifact with apply guidance', async () => {
    ...
    await screen.findByText('Runtime Config Alignment');
    await user.click(
      screen.getAllByRole('button', { name: 'Export core-backend env' })[0]
    );
    ...
    expect(
      await screen.findByText('Core-backend env exported')
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it('shows lifecycle readiness warnings without implying user conversations are blocked', async () => {
    ...
    const lifecycleStatus = await screen.findByRole('group', {
      name: 'Data Lifecycle Status',
    });
    expect(
      within(lifecycleStatus).getByText('Lifecycle Readiness: Stale')
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText(
  ...
  it('shows Deployment Readiness warnings and next actions', async () => {
    ...
    const readiness = await screen.findByRole('group', {
    ...
    expect(
      within(lifecycleItem as HTMLElement).getByText('Warning')
    ).toBeInTheDocument();
  ...
  it('guides admins through Deployment Wizard readiness review steps', async () => {
    ...
    await user.click(
      within(readiness).getByRole('button', { name: 'Open Readiness Review' })
    );
    ...
    expect(
      within(wizard).getByRole('link', {
        name: 'Review Inference Verification',
      })
    ).toHaveAttribute('href', '#inference-verification');
    ...
    ).toHaveAttribute('href', '#deployment-settings');
    ...
    expect(
      within(wizard).getByRole('link', { name: 'Review Operational Readiness' })
    ).toHaveAttribute('href', '#operational-readiness');
    ...
    ).toHaveAttribute('href', '#restart-required');
  ...
  it('shows current Model Provider verification status', async () => {
    ...
    const verification = await screen.findByRole('group', {
    ...
    expect(
      within(verification).getByText('https://inference.tinfoil.sh/v1')
    ).toBeInTheDocument();
    expect(
      within(verification).getByText(/Expected claims: expected-fingerprint/)
    ).toBeInTheDocument();
  });
  ...
  it('lets admins trigger manual inference verification', async () => {
    ...
    await user.click(
      within(verification).getByRole('button', { name: 'Verify Now' })
    );
    ...
    expect(
      await within(verification).findByText('Manual verification succeeded')
    ).toBeInTheDocument();
  });
  ...
  it('shows verification history and full attestation detail on demand', async () => {
    ...
    expect(
      within(verification).queryByText('full-attestation-material')
    ).not.toBeInTheDocument();

    await user.click(
      within(verification).getByRole('button', { name: 'Inspect Attestation' })
    );

    expect(
      await within(verification).findByText(/full-attestation-material/)
    ).toBeInTheDocument();
  ...
  it('shows operator-controlled privacy lifecycle status', async () => {
    ...
    expect(
      await screen.findByRole('group', { name: 'Data Lifecycle Status' })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText('Content Encryption Key: Configured')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Artifact Encryption Posture: Encrypted')
    ).toBeInTheDocument();
    expect(
      screen.getByText('Retention Scheduler: External or manual')
    ).toBeInTheDocument();
    expect(screen.getByText('Observation: Disabled')).toBeInTheDocument();
    expect(
      screen.getByText('Scheduler enabled classes: None')
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/Secure Erase is out of scope for v1/)
    ).toBeInTheDocument();
  ...
  it('lets admins mark uploaded artifacts as plaintext by operator choice', async () => {
    ...
    const lifecycleStatusCallsBeforeToggle = mockAdminFetch.mock.calls.filter(
      ([endpoint]) => endpoint === '/admin/lifecycle/status'
    ).length;
    await user.click(
      within(lifecycleStatus).getByRole('button', { name: 'Disabled' })
    );
    ...
    await waitFor(() => {
      const lifecycleStatusCalls = mockAdminFetch.mock.calls.filter(
        ([endpoint]) => endpoint === '/admin/lifecycle/status'
      );
      expect(lifecycleStatusCalls.length).toBeGreaterThan(
        lifecycleStatusCallsBeforeToggle
      );
    });
    expect(
      await within(lifecycleStatus).findByText(
        'Artifact Encryption Posture: Plaintext by Operator Choice'
      )
    ).toBeInTheDocument();
  });
  ...
  it('shows unsupported deployment surface categories and lets admins acknowledge one', async () => {
    ...
    expect(
      within(lifecycleStatus).getByText('Unsupported Deployment Surfaces')
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText('Runtime Logs')
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText('Database Internals')
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText('Docker Logs')
    ).toBeInTheDocument();

    const runtimeLogsCard = within(lifecycleStatus)
      .getByText('Runtime Logs')
      .closest('div.bg-surface');
    expect(runtimeLogsCard).not.toBeNull();
    await user.click(
      within(runtimeLogsCard as HTMLElement).getByRole('button', {
        name: 'Acknowledge',
      })
    );
    ...
    expect(
      await within(lifecycleStatus).findByRole('button', {
        name: 'Acknowledged',
      })
    ).toBeInTheDocument();
  });
  ...
  it('lets admins edit retention policy for a lifecycle data class', async () => {
    ...
    const sessionMemoryCard = within(lifecycleStatus)
      .getByText('Sage Session Memory')
      .closest('.bg-surface');
    ...
    expect(
      await within(lifecycleStatus).findByText(
        'Scheduler enabled classes: sage_session_memory'
      )
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText('Last run status: Succeeded')
    ).toBeInTheDocument();
  });
  ...
  it('lets admins preview retention, run scheduled retention, and see audit coverage', async () => {
    ...
    expect(
      within(lifecycleStatus).getByText(
        'Audit coverage: 6 audited, 2 exceptions, 0 missing.'
      )
    ).toBeInTheDocument();
    expect(
      within(lifecycleStatus).getByText('Scheduled classes: None')
    ).toBeInTheDocument();

    await user.click(
      within(lifecycleStatus).getByRole('button', { name: 'Preview Retention' })
    );
    expect(
      await within(lifecycleStatus).findByText(
        'Preview: 1 conversations, 2 document artifacts, 0 skipped classes.'
      )
    ).toBeInTheDocument();

    await user.click(
      within(lifecycleStatus).getByRole('button', { name: 'Run Scheduled' })
    );
    expect(
      await within(lifecycleStatus).findByText(
        'Scheduled run: succeeded with 1 tombstone retries.'
      )
    ).toBeInTheDocument();
  ...
  it('shows deletion tombstones and lets admins retry incomplete Session Memory deletion', async () => {
    ...
    const tombstones = await screen.findByRole('group', {
      name: 'Deletion Tombstones',
    });
    expect(
      within(tombstones).getByText('conversation-123')
    ).toBeInTheDocument();
    expect(
      within(tombstones).getAllByText('Incomplete').length
    ).toBeGreaterThan(0);
    expect(
      within(tombstones).getByText('target_unavailable')
    ).toBeInTheDocument();
    expect(within(tombstones).getByText(/Updated:/)).toBeInTheDocument();
    expect(
      within(tombstones).queryByText(/Deleted user conversation content/i)
    ).not.toBeInTheDocument();

    await user.click(
      within(tombstones).getByRole('button', {
        name: 'Retry deletion tombstone 7',
      })
    );

    await waitFor(() => {
      expect(mockAdminFetch).toHaveBeenCalledWith(
        '/admin/lifecycle/deletion-tombstones/7/retry',
        { method: 'POST' }
      );
    });
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(
        within(tombstones).getAllByText('Completed').length
      ).toBeGreaterThan(0);
    });
    expect(
      within(tombstones).queryByRole('button', {
        name: 'Retry deletion tombstone 7',
      })
    ).not.toBeInTheDocument();
  });
  ...
  it('shows an empty tombstone state when no lifecycle deletion needs retry', async () => {
    ...
    expect(
      within(tombstones).getByText(
        'No deletion tombstones are waiting for retry.'
      )
    ).toBeInTheDocument();
    expect(
      within(tombstones).queryByRole('button', {
        name: /Retry deletion tombstone/i,
      })
    ).not.toBeInTheDocument();
  });
  ...
  it('shows a tombstone fetch error instead of an empty state', async () => {
    ...
    expect(
      await within(tombstones).findByText('Unable to load deletion tombstones.')
    ).toBeInTheDocument();
    expect(
      within(tombstones).queryByText(
        'No deletion tombstones are waiting for retry.'
      )
    ).not.toBeInTheDocument();
  });
  ...
  it('refreshes deletion tombstones after a retry conflict', async () => {
    ...
    await waitFor(() => {
      const listCalls = mockAdminFetch.mock.calls.filter(
        ([endpoint]) => endpoint === '/admin/lifecycle/deletion-tombstones'
      );
  ...
  it('filters deletion tombstones by lifecycle status', async () => {
    ...
    expect(
      within(tombstones).getByText('conversation-456')
    ).toBeInTheDocument();

    await user.click(
      within(tombstones).getByRole('button', { name: 'Completed tombstones' })
    );

    await waitFor(() => {
      expect(mockAdminFetch).toHaveBeenCalledWith(
        '/admin/lifecycle/deletion-tombstones?status=completed'
      );
    });
    expect(
      within(tombstones).queryByText('conversation-123')
    ).not.toBeInTheDocument();
    expect(
      await within(tombstones).findByText('conversation-456')
    ).toBeInTheDocument();
  });
  ...
  it('keeps admin key migration behind a named destructive confirmation', async () => {
    ...
    await user.type(
      screen.getByLabelText('New admin public key'),
      'b'.repeat(64)
    );
    ...
    await waitFor(() => {
      expect(mockAdminFetch).toHaveBeenCalledWith(
        '/admin/key-migration/prepare'
      );
    });

    const warning = await screen.findByRole('note', {
      name: 'Admin key migration destructive warning',
    });
    expect(warning).toHaveTextContent('Confirm Key Migration');
    expect(warning).toHaveTextContent(
      'old key will no longer be able to decrypt instance data'
    );
    ...
    expect(
      within(dialog).getByRole('button', { name: 'Confirm & Migrate' })
