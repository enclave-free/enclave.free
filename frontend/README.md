...
React-based frontend for the Enclave RAG system.
...
| Key | Purpose |
|-----|---------|
| `enclave_admin_pubkey` | Admin's Nostr public key |
| `enclave_admin_session_token` | Admin session token |
| `enclave_user_email` | Verified user email |
| `enclave_user_name` | User's display name |
| `enclave_custom_fields` | Admin-configured custom fields schema |
| `enclave_user_profile` | User's completed profile data |
| `enclave_pending_email` | Email awaiting verification |
| `enclave_pending_name` | Name awaiting verification |
| `enclave_session_token` | User session token |
| `enclave_user_type_id` | Selected user type ID |
| `enclave_user_approved` | User approval status (see Security Note below) |
| `enclave_instance_config` | Instance branding configuration |
| `enclave_language` | User's selected language code (e.g., "en", "es", "ja") |

### Security Considerations

**Session Tokens**: Session tokens (`enclave_session_token`, `enclave_admin_session_token`) are stored in localStorage for simplicity. In XSS scenarios, an attacker could access these tokens. For production deployments with high security requirements, consider implementing httpOnly cookie-based sessions.

**Approval Status**: The `enclave_user_approved` localStorage value controls UI routing only. Approval status is validated server-side on each API request—unauthorized API access returns 403 Forbidden.
...
### Display Name
Custom name shown in headers and onboarding screens (default: "Enclave").
