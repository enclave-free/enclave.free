...
- `admin-config` is default-on for admin configuration conversations, which are config-only by default, while `knowledge-search`, `curated-resources`, `web-search`, and `db-query` remain explicit Admin controls.
