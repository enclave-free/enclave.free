...
Enclave uses a **Warm Neutral** color palette with configurable **accent colors**, designed to feel trustworthy, secure, and professional. The system supports both light and dark modes with smooth transitions.
...
1. Theme preference stored in `localStorage` as `enclave-theme`
