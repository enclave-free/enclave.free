...
The worst-case turn may now contain eight logical model requests and six executed Tool batches: six requests that select executable batches, a seventh request that selects the rejected batch, and one Tool-disabled final-answer request. Because each logical model request may receive one eligible same-model retry, the provider-request ceiling is sixteen attempts. With at most eight calls in each executed batch and one eligible retry for each safe read call, Tool execution is bounded at 48 logical calls and 96 attempts. The rejected seventh batch may contain at most eight correlated calls, but none execute. These are independent per-request and per-execution bounds, not one turn-wide retry. This remains intentionally smaller than an open-ended agent loop. When the bound is reached, operators receive a content-free `native_tool_round_limit_reached` event plus terminal rejected Tool evidence, while the model receives the correlated failures and remains responsible for the final answer.
