...
This document describes the two authentication systems in Enclave:
...
Enclave uses a two-tier authentication model:
...
**Requirements:**
- Kind must be `22242` (Enclave admin auth)
...
| `SMTP_FROM` | `Enclave <noreply@localhost>` | From address for emails |
...
| Key | Description |
|-----|-------------|
| `enclave_admin_pubkey` | Admin Nostr pubkey (after NIP-07 auth) |
| `enclave_user_email` | Verified user email |
| `enclave_user_name` | User display name |
| `enclave_user_type_id` | Selected user type during onboarding |
| `enclave_user_profile` | Complete user profile (JSON) |
| `enclave_user_approved` | Whether user is approved ("true"/"false") |
| `enclave_custom_fields` | Cached custom field definitions (JSON) |
| `enclave_pending_email` | Email awaiting magic link verification |
...
- `enclave_admin_session_token`
- `enclave_session_token`

---

## User Approval Workflow

Enclave supports optional manual approval of new users before they can access the system.
