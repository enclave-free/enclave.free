...
Each logical model request has one generic same-model retry budget. Alongside eligible transport, upstream, and protocol failures, complete provider silence for 20 seconds is a Pre-Response Provider Stall: Sage abandons that attempt and retries the identical request once. Any answer, reasoning, Tool-call, or other provider stream event makes that attempt ineligible for model-request retry. The retry cannot replay an executed Tool batch, and the existing 180-second attempt timeout remains for requests that have begun responding. The 20-second threshold is internal runtime policy rather than a new configuration surface.

When the provider attaches opaque Provider Continuity State to an assistant Tool-call message, Sage returns it unchanged to the same model with the correlated Tool results during the current bounded native loop. Sage does not interpret, stream, log, persist, export, or carry that state into another Conversation turn. It does not alter thinking mode, tune reasoning effort, add model-specific reasoning prompts, or synthesize state the provider did not supply.
...
Where supported, each streaming model request asks the provider for aggregate usage. Prompt, completion, total, cached, and reasoning counts are recorded only when the provider returns them; absent fields remain absent. These Model Usage Observations attach to the existing per-request timing trace, follow the Conversation Trace retention and deletion lifecycle, remain outside normal answer content, and do not create a separate billing or analytics subsystem. This operational Conversation metadata is available to Activity and Conversation Trace and is not Audit Log evidence.
