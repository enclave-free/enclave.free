...
The contact matrix is an explicit directory-value contract, not a real-world referral-quality test. Fixture version `neutral-contact-v4` asks for listed values as fictional test data in English and Spanish. The source retains its “do not contact” warning; an answer may quote the value with that caveat. Changed and unchanged journeys use the same prompts and receive no source-mutation hints. The manifest hashes the prompts and source description as well as the values. Historical `neutral-contact-v3` results remain unchanged and must not be pooled with new runs.

Inventory scorer `inventory-coverage-v3` recognizes full names, bounded ranges, and contiguous abbreviated suffix lists for the numbered synthetic inventory. It still requires successful Tool evidence, coverage of the requested records, terminal-page evidence for a complete set, and consistent count claims. Repeated names may be a recap; they cannot count as missing records. Invalid ranges and unrelated numbers do not supply coverage. A caveat about an earlier bounded page is distinguished from uncertainty about the complete retrieved set. Terminal-page paraphrases are accepted only with supporting Tool metadata. These are deterministic presentation checks, not semantic certification. The saved September 16 inventory answers are offline regressions; accepting them under the corrected checker is not a new live model run.
