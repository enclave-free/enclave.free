...
Inventory scorer `inventory-coverage-v2` recognizes full names, bounded ranges, and contiguous abbreviated suffix lists for the numbered synthetic inventory. It still requires successful Tool evidence, coverage of the requested records, terminal-page evidence for a complete set, and consistent count claims. Repeated names may be a recap; they cannot count as missing records. Invalid ranges and unrelated numbers do not supply coverage. These are deterministic presentation checks, not semantic certification. The saved September 16 inventory answers are offline regressions; accepting them under the corrected checker is not a new live model run.
