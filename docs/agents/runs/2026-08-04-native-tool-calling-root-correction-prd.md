...
- Verify the model may select multiple follow-up Tool batches and answer after their results. At the boundary, require exactly one content-free `native_tool_round_limit_reached` event; exactly one attempted event, rejected Tool-execution timing, and terminal rejected event for every correlated seventh-batch call; no successful execution evidence or backend invocation for that batch; and one final Tool-disabled request to the same model producing the answer.
