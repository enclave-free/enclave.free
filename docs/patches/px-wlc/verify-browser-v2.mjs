...
const out = resolve("docs/patches/px-wlc/screenshots/iteration-2");
...
try {
  ...
  await page.getByText("by Political Prisoners Support Team", { exact: true }).waitFor();
  assert(await page.getByAltText("World Liberty Congress").evaluate(el => el.complete && el.naturalWidth > 0));
  assert.equal(await page.getByText("How Enclave protects your data", {exact:true}).count(), 0);
  await shot("after-signup-desktop");
  for (const width of [1178, 901, 900, 768, 390, 320]) {
    await page.setViewportSize({width, height: 900});
    await noOverflow();
    const card = await page.locator('.px-onboarding .card').boundingBox();
    const footer = await page.locator('.px-entry-footer').boundingBox();
    assert(footer.y >= card.y && footer.y + footer.height <= card.y + card.height);
    await shot(`entry-${width}`);
  }
  await page.emulateMedia({colorScheme:'dark'});
  assert.equal(await page.locator('.px-onboarding .card').evaluate(el => getComputedStyle(el).backgroundColor), 'rgb(245, 249, 253)');
  ...
  await page.getByText('Admin Configuration Assistant', {exact:true}).waitFor();
  assert.equal(await page.locator('.px-welcome').count(), 0);
  const configCard = await page.locator('.max-w-5xl').boundingBox();
  assert(configCard.width > 800, `Admin content width ${configCard.width}`);
  await noOverflow();
  await shot('admin-config-with-assistant');
  check('Admin configuration has full content width and assistant, without welcome panel');
